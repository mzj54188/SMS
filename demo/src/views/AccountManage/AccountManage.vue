...
export default {
    ...
    methods: {
        ...
        getAccountListByPage () {
            ...
            // 发送ajax请求 把分页数据发送给后端
            this.axios.get('http://127.0.0.1:777/account/accountlistbypage', {
                params: {
                pageSize,
                currentPage
                }
        ...
        batchDelete(){
            ...
            .then(()=>{
                // 把删除账号数据的id发送给后端
                this.axios.get(`http://127.0.0.1:777/account/batchdelete`,{
                    params:{
                        selectedId
                    }
                })
        ...
        handleDelete(id){
            ...
            .then(()=>{
                this.axios.get(`http://127.0.0.1:777/account/accountdel?id=${id}`)
                .then(response=>{
                    // 接收后端返回的错误码 和 提示信息
                    let {error_code,reason}=response.data;
                    if(error_code===0){
        ...
        handleEdit(id){
            ...
            this.flag=true;
            // id发给后端
            this.axios.get(`http://127.0.0.1:777/account/accountedit?id=${id}`)
            .then(response=>{
        ...
        saveEdit(){
            ...
            // 把新数据 和 原来的id 发送给后端
            this.axios.post("http://127.0.0.1:777/account/accounteditsave",qs.stringify(params))
            .then(response=>{
                let {error_code,reason}=response.data;
                if(error_code === 0){
                    // 弹出成功的提示
